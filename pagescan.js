(() => {
  ...
  function captureFiguresFast(page, silent = false) {
    ...
    for (const region of regions) {
      let imgs = [];
      try {
        imgs = [...region.querySelectorAll("img")];
      } catch(e) {}
    ...
    if (!silent) {
      console.log(`\ud83d\udcf8 +${added} nya bilder p\u00e5 sida ${page}`);
    }
  }
  ...
  async function gotoPage(targetPage) {
    ...
    const prevPageNo = getPageNumber();

    if (String(targetPage) === String(prevPageNo)) return;
    ...
    for (let i = 0; i < 20; i++) {
      ...
      if (String(currentPage) === String(targetPage) || (currentSig && currentSig !== prevSig)) break;
    }
    await sleep(50);
  }
  ...
  async function gotoNextPageFast(mode = "text") {
    ...
    if (nextBtn) {
      ...
      if (input) {
        ...
        if (!isNaN(current)) {
          dispatchPageInput(input, current + 1);
        }
      }
    }
    ...
    for (let i = 0; i < polls; i++) {
      ...
      const pageChanged = String(currentPage) !== String(prevPage);
      const textChanged = currentSig && currentSig !== prevSig;
 
      if (pageChanged || textChanged) break;
    }
  ...
  async function autoCapture(startPage, endPage, mode = "text") {
    ...
    for (let step = 0; step < totalSteps; step++) {
      ...
      if (step > 0) {
        await gotoNextPageFast(mode);
      }
 
      captureCurrentPage({ includeFigures, silent: true });
 
      if (step % TURBO.renderEvery === 0 || step === totalSteps - 1) {
        renderList(true);
      }
    }
...
`;
    ...
    for (const page of sortedPages) {
      ...
      html += `
  <section class="page" id="page-${escapeHtml(page.page)}">
    <h2>${escapeHtml(page.title)}</h2>
    <div class="meta">Sida ${escapeHtml(page.page)}</div>
`;
      ...
      for (const para of paras) {
        ...
        for (const f of matched) {
          ...
          html += `
    <div class="figure">
      ${f.figureId ? `<div class="tag">${escapeHtml(f.figureId)}</div>` : ""}
      <img src="${escapeHtml(f.src)}">
      <div class="figcap">${escapeHtml(f.caption || `Bild p\u00e5 sida ${page.page}`)}</div>
    </div>
`;
        }
      }
...
<div class="grid">${cards}</div>
  ...
  function buildTxtExport() {
    ...
    for (const page of sortedPages) {
      ...
      if (last && last.title === page.title) {
        last.pages.push(page);
      } else {
        groups.push({ title: page.title, pages: [page] });
      }
    }
    ...
    pagesEl.innerHTML = sortedPages.length
      ? sortedPages.map(p => `
          <div style="padding:6px 8px;border-bottom:1px solid #1f2937;">
            ...
            <span style="color:#cbd5e1;">${escapeHtml(p.title)}</span>
          </div>
        `).join("")
    ...
    figsEl.innerHTML = sortedFigs.length
      ? sortedFigs.slice(0, 50).map((f, i) => `
          <div style="padding:6px 8px;border-bottom:1px solid #1f2937;">
            ...
            ${f.figureId ? ` \u2014 ${escapeHtml(f.figureId)}` : ""}
          </div>
        `).join("")
      : `<div style="opacity:.65;padding:12px;">Inga figurer f\u00e5ngade \u00e4n</div>`;
 
    statEl.innerHTML = `
      <strong>${state.pages.length}</strong> sidor \u00b7
      <strong>${state.figures.length}</strong> unika figurer
      ${state.autoRunning ? ` \u00b7 <span style="color:#86efac;">auto k\u00f6r (${escapeHtml(state.autoMode)})</span>` : ""}
    `;
  }
  ...
  panel.innerHTML = `
    <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;margin-bottom:12px;">
      ...
      <div id="stats" style="font-size:12px;color:#cbd5e1;"></div>
    </div>
    ...
    <div style="font-size:12px;color:#cbd5e1;background:#111827;padding:10px;border-radius:8px;margin-bottom:10px;line-height:1.4;">
      Ultra-turbo: auto f\u00e5ngar fram\u00e5t fr\u00e5n angiven startsida. Startsidan n\u00e4rs automatiskt f\u00f6re loopen b\u00f6rjar.
    </div>
    ...
    <div style="background:#111827;padding:12px;border-radius:10px;margin-bottom:10px;">
      ...
      <div style="display:flex;gap:8px;align-items:end;margin-bottom:8px;">
        <div style="flex:1;">
          <small>Fr\u00e5n sida</small><br>
          <input id="startPage" type="number" value="41" style="width:100%;padding:8px;background:#1e293b;border:1px solid #475569;border-radius:6px;color:#f8fafc;">
        </div>
        <div style="flex:1;">
          <small>Till sida</small><br>
          <input id="endPage" type="number" value="50" style="width:100%;padding:8px;background:#1e293b;border:1px solid #475569;border-radius:6px;color:#f8fafc;">
        </div>
      </div>
      ...
      <div style="display:flex;gap:8px;align-items:end;">
        <div style="flex:1;">
          ...
          <select id="modeSelect" style="width:100%;padding:8px;background:#1e293b;border:1px solid #475569;border-radius:6px;color:#f8fafc;">
            ...
            <option value="text+images">Text + bilder (lite l\u00e5ngsammare)</option>
          </select>
        </div>
        <button id="autoBtn" style="padding:10px 22px;background:#3b82f6;color:white;border:none;border-radius:8px;font-weight:700;">
          Starta auto
        </button>
      </div>
 
      <div style="font-size:11px;color:#94a3b8;margin-top:8px;">
        Bilder f\u00e5ngas bara i l\u00e4get \"Text + bilder\".
      </div>
 
      <button id="stopBtn" style="display:none;width:100%;margin-top:8px;padding:10px;background:#ef4444;color:white;border:none;border-radius:8px;">
        Stoppa auto
      </button>
    </div>
 
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:10px;">
      <button id="exportBtn" style="padding:12px;background:#a855f7;color:white;border:none;border-radius:8px;font-weight:700;">
        Visa export inline
      </button>
      <button id="galleryBtn" style="padding:12px;background:#2563eb;color:white;border:none;border-radius:8px;font-weight:700;">
        Visa galleri
      </button>
      <button id="txtBtn" style="padding:12px;background:#0ea5e9;color:white;border:none;border-radius:8px;font-weight:700;">
        Ladda ner TXT
      </button>
    </div>
 
    <button id="clearBtn" style="width:100%;padding:10px;background:#ef4444;color:white;border:none;border-radius:8px;margin-bottom:12px;">
      Rensa allt
    </button>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
      <div style="background:#111827;border-radius:10px;overflow:hidden;">
        ...
        <div id="pagesList" style="max-height:240px;overflow:auto;font-size:13px;"></div>
      </div>
      <div style="background:#111827;border-radius:10px;overflow:hidden;">
        <div style="padding:8px 10px;background:#1f2937;font-weight:700;">Figurer</div>
        <div id="figList" style="max-height:240px;overflow:auto;font-size:13px;"></div>
      </div>
    </div>
  `;
